...
public class MultiTouchController<T> {
    ...
    private void extractCurrentTouchPointInfo() {
        // Get new drag/pinch params. Only clientRead multitouch fields that are needed,
    ...
    interface OnDeleteListener {
        void onDelete(MultiTouchObject multiTouchObject);
    }

    public MultiTouchController(Context context, MultiTouchObjectCanvas<T> objectCanvas) {
        this(objectCanvas, true);
        this.gestureDetector = new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
            @Override
            public boolean onDoubleTap(MotionEvent e) {
                //Timber.d("OnDoubleTapListener... Has Selected Object: %s", selectedObject);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        try {
            ...
            for (int histIdx = 0; histIdx <= histLen; histIdx++) {
                ...
                // then clientRead from current event
    ...
    private void multiTouchController() {
        //Timber.d("Mode: %d isDown: %s", mMode, mCurrentTouchPoint.isDown());
        switch (mMode) {
            case MODE_NOTHING:
                if (mCurrentTouchPoint.isDown()) {
                    if(objectCanvas.isDeleteMode()) {
                        selectedObject = objectCanvas.getDeletableObjectAtPoint(mCurrentTouchPoint);
                    }
                    //Timber.d("Deletable Obj: %s", selectedObject);
                    if(selectedObject != null && onDeleteListener != null) {
                        onDeleteListener.onDelete(((MultiTouchObject) selectedObject));
                    } else {
